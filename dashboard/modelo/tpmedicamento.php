...
if ($modo==1)
{
	...
	$insert = "INSERT INTO tipo_medicamento(descripcion) VALUES (?)";
	$bd = 's';
	$val = array($des);
...
{
	$id = $_REQUEST["id"];
	$sql = "SELECT * FROM tipo_medicamento WHERE idtipo = ? ";
	$bindParam = 's';
	$value = array($id); 
	$rsl = $conecta->mysqli_prepared_query($sql,$bindParam,$value);  
	$id = $rsl[0]['idtipo']; 
	$des = $rsl[0]['descripcion']; 
	$modal = "<div class='modal fade' id='tpmedicamento$id' tabindex='-1' role='dialog' aria-labelledby='exampleModalLabel' aria-hidden='true'>
		    <div class='modal-dialog modal-lg' role='document'>
		      <div class='modal-content'>
		        ...
		        <div class='modal-footer'>
		          ...
		          <a class='btn btn-primary' href='#' onclick='act_tpmedica($id)'>
...
if ($modo==3)
{
	...
	$id = $_REQUEST["id"];
	$des = strtoupper($_REQUEST["des"]);
	$sql = "UPDATE tipo_medicamento SET descripcion=? WHERE idtipo=?";
	$bindParam = 'ss';
	$value = array($des,$id);
...
if ($modo==4)
{
	...
	$id = $_REQUEST["id"];
	$tari =  $conecta->fullselect("SELECT * FROM persona WHERE idtipo = $id");  
    $ro2 = $tari->num_rows;
    if ($ro2 > 0) {
    	echo "Error. este tipo ya se encuentra asignado a una persona!";
    } else {
    	$sql = "DELETE FROM tipo_medicamento WHERE idtipo = ? ";
		$bindParam = 's';
		$value = array($id); 
		$rsl = $conecta->mysqli_prepared_query($sql,$bindParam,$value);  
